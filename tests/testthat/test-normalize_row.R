...
test_that("normalize_row() stops when there are no possible references", {
  ...
  expect_error(normalize_row(lowest_level_df = lowest_level_df, exp_design = exp_design))
})

test_that("normalize_row() stops when user enters non-possible condition as ref", {
  data_path1 <- system.file("extdata", "proteinGroups.txt", package = "rowwisenorm")
  ...
  lowest_level_df <- return_list[["lowest_level_df"]]
  exp_design <- return_list[["exp_design"]]
  expect_error(normalize_row(lowest_level_df = lowest_level_df, exp_design = exp_design, ref = c("L", "M", "H")))
})
