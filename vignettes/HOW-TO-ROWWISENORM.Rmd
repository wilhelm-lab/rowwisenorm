...
```{r}
plot_results(lowest_level_df = lowest_level_df_norm)
```

Option 2: Set the name and the directory for the file
```{r}
plot_results(lowest_level_df = lowest_level_df_norm, main = "my_rowwisenorm_result", output_dir = "res")
```

Option 3: Set the directory with default name of the file 
```{r}
plot_results(lowest_level_df = lowest_level_df_norm, output_dir = "res")
```

Option 4: Set the name with the file being saved in current directory
```{r}
plot_results(lowest_level_df = lowest_level_df_norm, main = "my_rowwisenorm_result")
```
